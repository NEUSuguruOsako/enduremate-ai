...
import { idbPut, idbGet } from '../services/indexedDB'
...
// ========================
// 双层持久化：light 数据走 localStorage，heavy 数据走 IndexedDB
// ========================

const STORAGE_KEY = 'enduremate_data'

/** localStorage 只存储轻量数据（不含 timeSeries/laps/dynamics/parsedRecords） */
interface StoredDataLight {
  profile: UserProfile
  trainings: TrainingItem[]
  trainingRecords: Array<Omit<TrainingRecord, 'timeSeries' | 'laps' | 'dynamics' | 'injuryParts' | 'injurySeverity' | 'injuryDescription'>>
...
/** 按记录 ID 拆分存储的大字段 */
interface HeavyRecordFields {
  timeSeries: TrainingRecord['timeSeries']
  laps: TrainingRecord['laps']
  dynamics: TrainingRecord['dynamics']
  injuryParts: TrainingRecord['injuryParts']
  injurySeverity: TrainingRecord['injurySeverity']
  injuryDescription: TrainingRecord['injuryDescription']
}

function loadFromStorage(): StoredDataLight | null {
  try {
    const data = localStorage.getItem(STORAGE_KEY)
    if (data) {
      const parsed = JSON.parse(data) as StoredDataLight
      console.log(`[Persist] localStorage 加载 — profile:${!!parsed.profile?.name} records:${parsed.trainingRecords?.length ?? 0} plans:${parsed.trainings?.length ?? 0}`)
      return parsed
    }
    console.log('[Persist] localStorage 无 enduremate_data')
  } catch (err) {
    console.error('[Persist] 加载失败:', err)
  }
  return null
}

/** 仅保存轻量数据到 localStorage（同步） */
function saveToLocalStorage(data: StoredDataLight): boolean {
  try {
    const json = JSON.stringify(data)
    localStorage.setItem(STORAGE_KEY, json)
    const sizeKB = (new Blob([json]).size / 1024).toFixed(1)
    console.log(`[Persist] localStorage 保存成功 (${sizeKB}KB)`)
    return true
  } catch (err) {
    console.error('[Persist] localStorage 保存失败:', err)
    return false
  }
}

/** 将 heavy 字段异步保存到 IndexedDB */
async function saveHeavyToIndexedDB(records: TrainingRecord[]): Promise<void> {
  const items: Array<{ id: string; data: HeavyRecordFields }> = []
  for (const r of records) {
    if (r.timeSeries || r.laps || r.dynamics) {
      items.push({
        id: `record_${r.id}`,
        data: {
  ...
  if (items.length > 0) {
    console.log(`[Persist] IndexedDB 保存 ${items.length} 条记录的大字段`)
    // 逐条保存避免大事务失败
    for (const item of items) {
      await idbPut(item.id, item.data)
    }
  }
}

/** 从 IndexedDB 异步加载 heavy 字段并合并到轻量数据 */
async function hydrateFromIndexedDB(light: StoredDataLight): Promise<TrainingRecord[]> {
  const merged: TrainingRecord[] = []
  for (const record of light.trainingRecords) {
    const heavy = await idbGet<HeavyRecordFields>(`record_${record.id}`)
    merged.push({
      ...record,
      timeSeries: heavy?.timeSeries ?? [],
      laps: heavy?.laps ?? [],
      dynamics: heavy?.dynamics ?? null,
      injuryParts: heavy?.injuryParts ?? [],
      injurySeverity: heavy?.injurySeverity ?? null,
      injuryDescription: heavy?.injuryDescription ?? '',
    } as TrainingRecord)
  }
  return merged
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  //   trainingRecords 初始不含 timeSeries 等大字段，由 useEffect 异步从 IndexedDB 补全
  ...
  const [trainingRecords, setTrainingRecords] = useState<TrainingRecord[]>(
    (storedData?.trainingRecords ?? []) as TrainingRecord[]
  )
  ...
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>(
    (storedData?.uploadedFiles ?? []) as UploadedFile[]
  )
  ...
  // ---- 初始化：从 IndexedDB 异步加载 heavy 字段（timeSeries/laps/dynamics） ----
  useEffect(() => {
    if (storedData && storedData.trainingRecords.length > 0) {
      console.log(`[Persist] 开始从 IndexedDB 水合 ${storedData.trainingRecords.length} 条记录的大字段...`)
      hydrateFromIndexedDB(storedData).then((merged) => {
        console.log(`[Persist] IndexedDB 水合完成 — ${merged.filter(r => r.timeSeries?.length).length} 条含图表数据`)
        setTrainingRecords(merged)
        setIdbHydrated(true)
        if (merged.length > 0) {
          setMetrics(calculateMetrics(merged, storedData.injuryRecords || []))
        }
      }).catch((err) => {
        console.error('[Persist] IndexedDB 水合失败:', err)
        setIdbHydrated(true) // 即使失败也标记完成，使用轻量数据
      })
    } else {
      setIdbHydrated(true) // 无记录可水合
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (!idbHydrated) {
      console.log('[Persist] IndexedDB 尚未水合完成，跳过保存')
      return
    }

    console.log(`[Persist] 触发保存 — profile:${!!profile.name} records:${trainingRecords.length} plans:${trainings.length}`)

    // 1. 构建轻量数据保存到 localStorage
    const lightData: StoredDataLight = {
      profile,
      trainings,
      trainingRecords: trainingRecords.map((r) => {
        const { timeSeries, laps, dynamics, injuryParts, injurySeverity, injuryDescription, ...rest } = r
        return rest
      }),
      injuryRecords,
    ...
    const saved = saveToLocalStorage(lightData)

    // 2. 异步保存 heavy 字段到 IndexedDB
    saveHeavyToIndexedDB(trainingRecords)

    if (!saved) {
      if (!lastSaveErrorRef.current) {
        lastSaveErrorRef.current = true
        notifyRef.current.error('数据保存失败', '浏览器存储空间不足，请清理缓存后重试')
      }
    } else {
      lastSaveErrorRef.current = false
    }
  }, [profile, trainings, trainingRecords, injuryRecords, uploadedFiles, currentPhase, currentWeek, planAutoGenerated, autoGenReason, idbHydrated])
