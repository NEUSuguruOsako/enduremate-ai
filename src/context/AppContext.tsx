...
function saveToStorage(data: StoredData): SaveResult {
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'QuotaExceededError') {
      console.warn(`[Storage] 完整数据过大 (${sizeKB.toFixed(0)}KB)，降级存储...`)
    } else {
    ...
    try {
      ...
      console.error('[Storage] 精简版也无法保存:', e2)
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  }, [profile, trainings, trainingRecords, injuryRecords, uploadedFiles, currentPhase, currentWeek, planAutoGenerated, autoGenReason, notify])
